    ...
    return (
        <div style={{ display: "flex", justifyContent: "center", height: "400px", alignItems: 'center' }}>
            <Card className="site-card-border-less-wrapper" size="default">
                {tasks.length == 0 ? <CreateTask allTasks={tasks} completeAllTasks={completeAllTasksFunc} addTask={addTask} /> :
                    <List
                        size="large"
                        header={<CreateTask allTasks={tasks} completeAllTasks={completeAllTasksFunc} addTask={addTask} />}
                        footer={<Footer tasksRemaining={tasksRemaining} removeCompleted={removeCompleted} type={type} _type={_type} />}
                        bordered={true}
                        dataSource={type == 'P'?tasks.filter(task=>!task.completed):type == 'C'?tasks.filter(task=>task.completed):tasks}
                        renderItem={item =>
                            <List.Item>
